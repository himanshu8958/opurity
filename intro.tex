...
is returned in Line 12. However, to establish that Line~8 also returns n!, an
...
returns n * \emph{factCache}(n-1). Due to the transformation to Line~10 mentioned above,
(b) the theorem prover can infer that Line~12 also returns n *
\emph{factCache}(n-1). Note that since these two expressions are syntactically
...
`(g = -1) $\vee$ (g = lastN * \emph{factCache}(lastN-1))' that is easier to infer by a
