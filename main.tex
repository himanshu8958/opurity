...
`$n = 3$' it may return 1 or 3!, thus for this set of glboal states it
is not OP.  We assume that this set of global states can be
...
-1 \vee g = 6$ and the function `factorial' can be modelled as
$\fact[k] == 1 \wedge k >1 ==> \fact[k] == \fact[k-1]*k$. Thus, the
pre-condition is $\inv$ and the post-condition is $\inv \wedge
\fact[k] == 1 \wedge k >1 ==> \fact[k] == \fact[k-1]*k$.

Now, In general the above mentioned idea is feasible. But there are
certain issues (a) It is hard for theorem provers in general to prove
such assertions (b) describing the procedure in logic takes
considerable effort. We propose that instead of trying to describe the
function for the given procedure, we abstract out the mathematical
function using an uniterpreted function symbol. And then ask the
question whether an assignment for the function symbol exists while
constraining its meaning based on the given procedure. Now, this kind
of a query is also hard for theorem provers but we showcase another
technique that borrows from this idea, which is able to prove
factorial to be OP.

This idea of abstracting using a mathematical function symbol also
comes in hand for the invariant generation. if the result from
a recursive procedure call flows to a global variable, the invariant
computation can also represent the recursive call as the same function
symbol as above.
...
invariant we used is $\g = -1 \vee g = \F(2) * 3$. The symbol
`$\F(2)$' in the invariant represents the return value from the
procedure `factorial' assuming it is a mathematical function. The
