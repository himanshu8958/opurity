...
\begin{definition}[invariant]
  ...
  Similar to requires and ensures, $\inv$ is assumed at the beginning
  of a procedure and ensured at the end. Also, ensured before a
  procedure call and assumed after it.
  The uninterpreted function symbol $\F$ is a placeholder for the
  mathematical function equivalent to the given procedure. Thus, this
  invariant assumes that the given procedure is a mathematical
  function and is undefined otherwise.
...
\begin{definition}[observational purity(invariant $\inv$)]
 ...
 %% procedure return the same result if the traces started with the same
 %% argument value.
 %% argument value is the same.
...
$\formula_e$ has a single free variable, $\F$ and it is equivalent to
the return value from the procedure for all values of the parameter
`n' and all values of the global variable `g' at the boundaries.
Thus, if $\formula_e$ is SAT, it implies that there exists a function
which mimics the given procedure in all cases.
...
the ensures statements is verified. The statements `ensures x' and
...
$\neg$x is reachable at the assert statment. In our analysis, we check
