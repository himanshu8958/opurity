...
%% There will be two blank lines before and after the Abstract. \dots
  ...
  (option 1)\\
  A procedure that is a pure function of the arguments, modulo side
  ...
  automatically check OPness for recursive procedures.\\

  (option 2)\\ A specification language that includes procedure calls
  will greatly improve ease of expression. JML currently allows only
  pure functions to be used in the specifications. We give A procedure
...
of the procedure is independent of any global state. This has many
applications.  For instance, implementations of dynamic programming
problems without the memoization would mimic the input-output
behaviour as of the optimal implementations. Checking whether a given
procedure is OP, is undecidable. In this paper, we propose a pair of
static analysis techniques to check OPness that are sound but
incomplete.

All logical and arithmetic operators are mathematical functions
(function), and functional composition is closed under the set of
functions. 
%% this property of being a function is closed under
%% composition.
...
OPness is a property of a procedure and the respective reachable
global states.  Procedure `factorial' in
...
3$' it may return `1' or `3!'.  We assume that this set of global
states can be represented in the underlying logic and we refer to it
as \textit{invariant}.  The global states reachable at all boundaries,
of each procedure call, of the given procedure must satisfy the
invariant.
...
To check if a given procedure is OP, we ensure that for all
...
the post condition can be substituted by the invariant ( in-order to
verify the invariant). This does
not constrain the return value of the procedure. However, the return value
...
then the procedure is OP otherwise, it is not OP.
...
One direct application of this work is automatic memoization.
...
solution when needed, thus trading memory for compute time. Our
...
assigns any value to variable `x' from it's declared domain. And the
statement `assume x' allows the execution of the program to proceed if
the predicate `x' holds at the program point, otherwise it halts the
execution.
...
We transform the given procedure into the following representation to
be able to represent it in logic. For example procedure `factorial' in
...
\item Continuation of above, recursive calls may modify global
  ...
  global variable accessed by the given program.
\item We add the statement `assume x = $\F$(y)' after the statement
  `havoc(x)'.
\item Next, we add extra variables to store the value of the global
  variables at the procedure boundaries. Before the $\mathit{i^{th}}$
...
We represent the given procedure in logic, such that it captures the
value of the argument, the return value and the values of the global
variables at the boundaries. We track the values of the global
variables using extra variables as explained in
...
this suffices safety for our analysis.
