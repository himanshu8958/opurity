...
%% There will be two blank lines before and after the Abstract. \dots
  ...
  %% (option 2)\\
  A specification language that includes procedure calls
  will greatly improve ease of expression. JML currently allows only
  pure procedures to be used in the specifications. This is overly
  restrictive, as the procedure might have benign side effects, which
  are not observable to other procedures.
...
\keywords{specification languages, documentation tools, observational
  purity, pure method, formal methods, program verification,
...
Thus ensuring the existence of a mathematical-function (function) that
mimics the given procedure. Implying, input-output behaviour of the
procedure is independent of any global state. This has many
applications. For instance, implementations of dynamic programming
problems without memoization would mimic the input-output
behaviour of the implementations with memoization. Checking whether a given
...
fact[n]$' where $\retVar$ is the return variable.
...
representing the post-condition in an easy manner and also it aids in
...
assuming it is a mathematical function. Note that the invariant used
...
`\retVar' (line 4). Also, we have an added statement (li5) in
...
Naumann~\cite{naumann2007observational} like
\cite{barnett2006allowing} call a procedure OP if it is
output-equivalent to a procedure which is a pure function (without
side-effects). These works present/use a theory for simulations,
closely related to ownership types.

Cok~\cite{cok2008extensions} take a different approach of restricting
