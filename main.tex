...
Defining the correctness for a program is a time consuming task that
requires deep understanding of the expected functionality of a
program. In this paper, we talk about a general property that is the
...
In the absence of global variables, since all the operators (logical
and arithmetic) are mathematical functions, their composition is also
a mathematical function (function). Hence the procedure is a function
of the formal arguments. But, OP procedures that access global
variables make more interesting examples, for instance procedure
`factorial' in Listing~\ref{lst:factorialSimple}. In order to prove
the result form a procedure is independent of the global values, we
prove that for all reachable global state, for each argument value
there is a single result. This invariant over the global variables,
can optimistically assume that the given procedure is observationally
pure. This is make it feasible to generate an invariant for the given
procedure.
...
Dynamic programming algorithms use memoization to store pre-computed
solutions to the sub-problem so that it can look-up the solution when
needed, thus trading compute time for storage. Our technique can check
if it is safe the memoize the results produced by a procedure. By
safety we mean assurance that the procedure will produce the same
result it returned earlier. We propose two static analysis techniques
to check this problem. The property we check is whether the given
procedure's behaviour is equivalent to some mathematical function.
One interesting thing about our approach is that the user need not
mention a (the) mathematical function that the procedure is supposed
to mimic.
...
the following set of free variables \{$g, gout, n, retVar, gbef_1,
\cdots gbef_m, gaft_1, \cdots gaft_m$\}. Variable $g$ represents the
value of the global variable `g' at the beginning of procedure `\foo'
whereas $gout$ represents value of `g' at the end of `\foo'. Variable
$n$ represents the value of the argument, $retVar$ the return
value. And $gbef_i$ and $gaft_i$ represent the value of `g' before and
after the $i^{th}$ procedure call statement and there are $m$
...
\section{Static analysis approaches}
