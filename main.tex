...
Specification languages for imperative languages such as Spec\#, JML,
Eiffel allow procedure calls in contracts of a procedure.  As always,
the presence of the specification must not influence the behaviour of
the procedure.
%% The
...
procedures must be restricted. Strong purity allows no side effects
...
function. This paper describes a pair of techniques to check if a given procedure
implements a mathematical function. If the return value from a
procedure is dependent only on the formal arguments, irrespective of
whether the procedure reads/writes global variables, 
% even if the
...
behaviour of $\foo$ in all sequence of procedure calls to $\foo$ , where 
both $\foo$ and $\F$ share the same signature.  Thus, the input-output
behaviour of  observationally pure procedures is independent of the global
variables. This property is a generalization of referential transparency.
%% For instance,
...
% value of
% the argument value, the return value and the values of the global variables
% at the boundaries
