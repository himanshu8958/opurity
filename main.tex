...
set $\{g = -1, g = 3!\}$, procedure `factorial' mimics $\lambda
...
of the given procedure must satisfy the invariant.
%% Also
...
not constrain the return value of the procedure. And the return value
from an inner recursive call might flow into a global variable. Thus,
...
takes considerable effort. We propose, instead of trying to formalize
...
yes, then the procedure is OP. This is our existential approach.
...
This idea of abstracting using a mathematical function symbol goes a
long way, also in describing the invariant on the global states.
...
For procedure `factorial' we
can use the invariant `$\inv_{\F} : g = -1 \vee g = \F(2) * 3$'. The
symbol `$\F(2)$' in the invariant represents the return value from the
procedure `factorial' assuming it is a mathematical function. Note that
invariant used here is valid iff the given procedure is OP, otherwise
it is not well defined.

%% should I talk about why F(2) *3 instead of F(3) ?

Another technique to check OPness is to represent two instances of a
procedure in some underlying logic. Then ask the question that do the
two instances of the given procedure always mimic each other? If
yes, then we show that the given procedure is OP. Otherwise, we are
able to construct a witness to the impurity of the given procedure. We
construct a pair of traces, such that both begin in a global state in
the invariant, the same argument value but return different
results. We call this the `impurity witness approach'.
...
for constructing the invariant in Section~\ref{sec:invGen}.
...
non-termination there isn't a return value.
...
procedure. By safety, we mean preservation of the input-output
