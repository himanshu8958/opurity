...
\begin{definition}[observational purity(set of global state)]%%  A procedure `\foo' is 
  %% observational pure wrt a given set of global states $\Gstates$ if
  %% for each history $\history$ of procedure `\foo', all pairs of traces
  %% in the history that begin in global state $\gstate$ respect each
  %% other where $\gstate \in \Gstates$.  In other words $\forall \gstate
