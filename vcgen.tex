...
\[
\begin{array}{lll}
\postfn(\pre, \code{x = e}) &= (\exists x. \pre) \wedge (x = e) & \text{if } \code{x} \not\in \text{vars}(\code{e}) \\
\postfn(\pre, \code{havoc(x)}) &= \exists x. \pre \\
...
\vcfn(\pre, \stmt) &= \text{true} & \text{for all other statements \stmt}
\end{array}
\]
...
Finally, we compute a verification condition $\vc$ that represents the conditions we must check to ensure that
...
Finally, we add the check that the initial state too must satisfy $\inv$ as the basis condition for
induction.
